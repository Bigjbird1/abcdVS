...
export default function ProfileSetup() {
  ...
  // Type guards
  const isBuyerData = (data: BuyerFormData | SellerFormData): data is BuyerFormData => {
    return userType === 'buyer';
  };

  const isSellerData = (data: BuyerFormData | SellerFormData): data is SellerFormData => {
    return userType === 'seller';
  };

  const onSubmit: SubmitHandler<BuyerFormData | SellerFormData> = async (data) => {
    setLoading(true);
    setError('');
      ...
      return (
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label htmlFor="phoneNumber" className="block text-sm font-medium mb-1.5">Phone Number</label>
    ...
    if (step === 2 && userType === 'seller') {
      return (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="space-y-4">
            <div>
              <label htmlFor="governmentId" className="block text-sm font-medium mb-1.5">Government ID</label>
              <input
                type="file"
                id="governmentId"
                className="hidden"
              ...
              />
              <label htmlFor="governmentId" className="w-full p-4 border-2 border-dashed rounded-lg hover:border-gray-400 cursor-pointer">
                <div className="flex flex-col items-center gap-2">
                  <Upload className="w-6 h-6 text-gray-400" />
                  <span className="text-sm text-gray-600">Upload a photo of your ID</span>
                </div>
              </label>
              {userType === 'seller' && 'governmentId' in errors && errors.governmentId && <p className="text-red-500 text-sm mt-1">{errors.governmentId.message}</p>}
            </div>

            <div>
              <label htmlFor="venueContract" className="block text-sm font-medium mb-1.5">Venue Contract</label>
              <input
                type="file"
                id="venueContract"
                className="hidden"
              ...
              />
              <label htmlFor="venueContract" className="w-full p-4 border-2 border-dashed rounded-lg hover:border-gray-400 cursor-pointer">
                <div className="flex flex-col items-center gap-2">
                  <Upload className="w-6 h-6 text-gray-400" />
                  <span className="text-sm text-gray-600">Upload your venue contract</span>
                </div>
              </label>
              {userType === 'seller' && 'venueContract' in errors && errors.venueContract && <p className="text-red-500 text-sm mt-1">{errors.venueContract.message}</p>}
            </div>
    ...
    if (step === 2 && userType === 'buyer') {
      return (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="space-y-4">
            <div>
              <label htmlFor="preferredStartDate" className="block text-sm font-medium mb-1.5">Preferred Date Range</label>
              <div className="grid grid-cols-2 gap-4">
                <div className="relative">
            ...
            <div>
              <label htmlFor="guestCount" className="block text-sm font-medium mb-1.5">Guest Count</label>
              <input
                type="number"
                id="guestCount"
                className="w-full px-3 py-2 border rounded-lg"
              ...
              />
              {userType === 'buyer' && 'guestCount' in errors && errors.guestCount && <p className="text-red-500 text-sm mt-1">{errors.guestCount.message}</p>}
            </div>
          </div>
