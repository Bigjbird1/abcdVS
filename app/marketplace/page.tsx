...
const MarketplaceBrowse = () => {
  ...
  const conditions = ['New', 'Like New', 'Excellent', 'Good', 'Made to Order', 'Custom Made', 'Handmade'];
  ...
  const activeFilters = useMemo(() => {
    ...
    if (priceRange) {
      const rangeLabel = priceRanges.find(r => 
        r.range[0] === priceRange[0] && r.range[1] === priceRange[1]
      )?.label;
      filters.push({ type: 'price', value: rangeLabel || `$${priceRange[0]} - $${priceRange[1]}` });
    }
    if (selectedCondition) filters.push({ type: 'condition', value: selectedCondition });
    return filters;
              ...
              {activeFilters.map((filter, index) => (
                ...
                >
                  ...
                  <button
                    onClick={() => {
                      switch (filter.type) {
                        case 'size': setSelectedSize(null); break;
                        case 'price': setPriceRange(null); break;
                        case 'condition': setSelectedCondition(null); break;
                      }
                    }}
